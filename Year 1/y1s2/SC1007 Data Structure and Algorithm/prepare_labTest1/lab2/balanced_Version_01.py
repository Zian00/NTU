...
def balanced(expression):
    ...
    for char in expression:
        if char in check_symbol.keys():
            s.push(char)
            # print(f"keys here {char}\n")
        elif char in check_symbol.values():
            # there's closing but stack is empty
            # or the opening bracket is not corresponding to the closing bracket
            if s.isEmpty() or check_symbol[s.pop()] != char:
                return False
            # s.pop()
            # print(f"values here {char}\n")
