...
def moveOdditemstoback(head):
 # Write your code here #
    ...
    while current:
        if current.data % 2 ==0 : # even number
            if even_head is None:
                ...
                even_tail = current # re-assign current to be even_tail
        else:
            if odd_head is None:
    ...
    # check if even list exist
    if even_head is None:
        return odd_head
    
    # check if odd list exist
    if odd_head is None:
        return even_head
    
    # link the two lists up
    even_tail.next = odd_head

    if odd_tail:
        odd_tail.next = None # set last node point to None
