...
def reverseDoublyList(head):
    ...
    if current is None or current.next is None:
        return current
    while current:
        # store current next pointer to tmp
        temp = current.next
        # current next pointer point to current previous
        current.next = current.prev
        # current previous pointer to original current next
        current.prev = temp
        # set new head as the current
        new_head = current
        print(f"new head: {new_head.data}")
        # move to next node, which is current prev
