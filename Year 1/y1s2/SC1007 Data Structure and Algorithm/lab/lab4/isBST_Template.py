...
def insertBSTNode(root, value):
    ...
    # when exiting the while loop means current has reached None for inserting the new value
    # The parent variable now holds the node that will become the parent of the new node.
...
def isBST(node, min_val=float('-inf'), max_val=float('inf')):
    ...
    # returns True when both left subtree and right subtree satisfy the BST conditions
